...
$list_cnt = count($list); // $list 변수는 plugin/nariya/bbs/list.php (각 게시판 글목록을 만드는 플러그인 파일), theme/damoang/skin/board/basic/list.skin.php 에 있음.
...
function get_wr_class_and_set_row_f20240617(&$row, $wr_id)
{
    $li_css = '';
    if ($row['wr_7'] == 'lock')
    { // 잠금(wr_7)
...
/** 그리드용 label의 글자를 표시하고 $row 참조값 변경 */
...
function get_attachment_icon_f20240617($row, $has_video, $has_img)
{
    $iconHtml = ''; // 구: $wr_icon
