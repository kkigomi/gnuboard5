...
<article id="bo_v" class="mb-4">
	...
	<section id="bo_v_info" class="f-sm font-weight-normal mb-3 mb-sm-4">
		...
		<?php if($is_reading > 1) { ?>
			<div class="alert alert-warning mx-3 mx-sm-0 mt-2" role="alert">
				<?php echo date("Y년 m월 d일 H시 i분", $is_reading) ?>까지 열람할 수 있습니다.
			</div>
		<?php } ?>
		<?php
			// 내용 상단 컨텐츠
			...
			<?php } else { ?>
				<div class="row justify-content-center mt-3 mb-4">
					<div class="col-sm-8 col-lg-6">
						<div class="card reading-preview">
							<?php if($view['seo_img']) { ?>
								<style>
									.reading-preview img { width:100%; max-width:100%; }
								</style>
								<?php echo get_view_thumbnail('<img src="'.$view['seo_img'].'">') ?>
							<?php } ?>
							<div class="card-body text-center">
								<p class="card-text">
									열람시 <b><?php echo number_format($view['as_view']) ?></b> 포인트 차감
									<?php if($is_reading_term) { ?>
										/ <b><?php echo number_format($is_reading_term) ?></b>일간 열람이력 유지
									<?php } ?>
								</p>
								<p class="pt-3">
									<button class="btn btn-primary btn-lg btn-reading">
				...
				$(function() {
					$(".btn-reading").click(function() {
						if (confirm("열람하시겠습니까?")) {
							$.ajax({
								type: "POST", 
								url: "<?php echo $board_skin_url ?>/ajax.read.php",
