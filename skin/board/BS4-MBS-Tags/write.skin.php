...
if(isset($boset['na_tag']) && $boset['na_tag']) {
	...
	$stag = array();
	$is_sel_stag = false;
	if(isset($write['as_tag']) && $write['as_tag']) {
...
<section id="bo_w" class="f-de font-weight-normal mb-4">
	...
	<li class="list-group-item">
	   <span class="sr-only">내용<strong>필수</strong></span>
		...
		?>
			<input type="hidden" name="as_tag" id="as_tag" value="">
					...
					<div class="d-flex">
						<div id="bo_tag_list" class="sly-wrap flex-grow-1">
							<ul id="bo_tag_ul" class="sly-list nav d-flex border-left-0 text-nowrap" role="tablist">
								<li class="nav-item">
									<a class="nav-link py-2 px-3"><b>태그 선택</b></a>
								</li>
			...
			<div class="tab-content p-3 border border-top-0 mt-0 mb-0 f-de">
				<?php for($i=0; $i < $tlist_cnt; $i++) { ?>
					<div class="tab-pane<?php echo (isset($tlist[$i]['on']) && $tlist[$i]['on']) ? ' show active' : '';?>" id="tags_<?php echo $i ?>" role="tabpanel" aria-labelledby="tag_tab_<?php echo $i ?>">
						...
						for($k=0; $k < $stags_cnt; $k++) {
							$stags_checked = ($is_sel_stag && in_array($stags[$k], $stag)) ? ' checked' : '';
						?>
							<div class="col px-1">
	...
	<?php if(isset($boset['na_tag']) && $boset['na_tag']) { // 추가태그 - wr_10 필드로 값을 옮김 ?>
		<li class="list-group-item">
			<div class="form-group row mb-0">
				<label class="col-md-2 col-form-label" for="wr_10">추가 태그</label>
				<div class="col-md-10">
					<input type="text" name="wr_10" id="wr_10" value="<?php echo isset($write['wr_10']) ? $write['wr_10'] : ''; ?>" class="form-control" placeholder="콤마(,)로 구분하여 복수 태그 추가 등록 가능">
				</div>
			</div>
		</li>
	<?php } ?>	
