...
<section id="bo_w" class="f-de font-weight-normal mb-4">
	...
	<?php
		...
		$real_name_txt = (isset($boset['nuse']) && $boset['nuse']) ? '익명' : '실명'; 
		$real_name_checked = (isset($write['wr_7']) && $write['wr_7']) ? ' checked' : '';
		$option .= '<div class="custom-control custom-checkbox custom-control-inline">'.PHP_EOL;
		$option .= '<input type="checkbox" name="wr_7" value="1" id="wr_7" class="custom-control-input" '.$real_name_checked.'>';
		$option .= '<label class="custom-control-label" for="wr_7"><span>'.$real_name_txt.'</span></label>';
		$option .= '</div>'.PHP_EOL;
	...
	<li class="list-group-item">
		<div class="form-group row mb-0">
			<label class="col-md-2 col-form-label" for="as_view">열람 점수</label>
			<div class="col-md-3">
				<div class="input-group">
					<input type="text" name="as_view" id="as_view" value="<?php echo isset($write['as_view']) ? $write['as_view'] : ''; ?>" class="form-control" placeholder="양수 입력">
			...
			<?php 
				$boset['rrp'] = (isset($boset['rrp']) && (int)$boset['rrp'] > 0) ? (int)$boset['rrp'] : 0;
				$boset['nrp'] = (isset($boset['nrp']) && (int)$boset['nrp'] > 0) ? (int)$boset['nrp'] : 0; 
				$boset['xrp'] = (isset($boset['xrp']) && (int)$boset['xrp'] > 0) ? (int)$boset['xrp'] : 0;
			?>
			<?php if($boset['rrp']) { ?>
				<div class="col-md-7">
					<p class="form-control-plaintext text-muted">
						설정 점수의 <?php echo $boset['rrp'] ?>% 적립
					</p>
				</div>
			<?php } else if ($boset['nrp'] || $boset['xrp']) { ?>
				<div class="col-md-7">
					<p class="form-control-plaintext text-muted">
						열람 점수는
						<?php if($boset['nrp']) { ?>
		...
		<?php if(!$boset['rrp'] && ($boset['nrp'] || $boset['xrp'])) { ?>
			<div class="form-group row mb-0">
				<div class="col-md-10 offset-md-2 text-muted">
					열람 점수는
					<?php if($boset['nrp']) { ?>
						<?php echo number_format($boset['nrp']) ?> 이상
					<?php } ?>
	...
	<?php if(!$is_admin) { ?>
		...
		<?php if($boset['nrp'] > 0) { ?>
			if(rpoint < <?php echo $boset['nrp'] ?>) {
				alert("<?php echo $boset['nrp'] ?>포인트 이상 설정하셔야 합니다.");
				f.as_view.focus();
				return false;
			}
		<?php } ?>
		<?php if($boset['xrp'] > 0) { ?>
			if(rpoint > <?php echo $boset['xrp'] ?>) {
				alert("<?php echo $boset['xrp'] ?>포인트 이하로 설정하셔야 합니다.");
				f.as_view.focus();
				return false;
			}
		<?php } ?>
