...
// 원글 작성자
$wr_mb_id = ($view['mb_id']) ? $view['mb_id'] : $view['wr_8'];
	...
	<section id="bo_vc" class="na-fadein">
		...
		for ($i=0; $i<$cmt_amt; $i++) {
			...
			*/
			...
			$cmt_mb_id = ($list[$i]['mb_id']) ? $list[$i]['mb_id'] : $list[$i]['wr_8'];
			$by_writer = ($wr_mb_id && $wr_mb_id == $cmt_mb_id) ? ' by-writer' : '';
		 ?>
		...
		<aside id="bo_vc_w">
			...
			<div class="cmt-box p-3 bg-light border na-round">
				...
				<ul class="d-flex align-items-center f-sm">
					...
					<li class="text-nowrap pr-2">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" name="wr_7" value="1" id="wr_7" class="custom-control-input">
							<label class="custom-control-label" for="wr_7"><span><?php echo (isset($boset['nuse']) && $boset['nuse']) ? '익명' : '실명';?></span></label>
						</div>
					</li>
