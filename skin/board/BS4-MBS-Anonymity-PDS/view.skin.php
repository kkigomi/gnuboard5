...
// 다운로드 포인트
$board['bo_download_point'] = (int)$write['wr_1'] * (-1);

// 다운 유지
$down_term = isset($boset['down_term']) ? (int)$boset['down_term'] : 0;
$is_down_term = '';
if ($is_member && $board['bo_download_point'] && $down_term) {
	$sql_term = na_sql_term($down_term, 'po_datetime'); // 기간(일수,today,yesterday,month,prev)
	$row = sql_fetch(" select po_datetime from {$g5['point_table']} where mb_id = '{$member['mb_id']}' and po_rel_table = '$bo_table' and po_rel_id = '$wr_id' and po_rel_action = '다운로드' $sql_term order by po_datetime desc");
	if(isset($row['po_datetime']) && $row['po_datetime']) {
		$is_down_term = date("Y년 m월 d일 H시 i분", (strtotime($row['po_datetime']) + ($down_term * 86400)));
	}
}
...
<article id="bo_v" class="mb-4">
	...
	<section id="bo_v_info" class="f-sm font-weight-normal mb-3 mb-sm-4">
		...
		<?php
			// 내용 상단 컨텐츠
			if(isset($boset['contm']) && $boset['contm']) {
				@include($board_skin_path.'/content/'.$boset['contm'].'.php');
			}
		?>
	</section>
    ...
    <section id="bo_v_atc">
		...
		<?php
			// 내용 하단 컨텐츠
			if(isset($boset['contb']) && $boset['contb']) {
				@include($board_skin_path.'/content/'.$boset['contb'].'.php');
			}
		?>
	</section>
			...
			<div class="d-table-cell px-3 py-2">
				<?php if($board['bo_download_point']) { ?>
					<div class="d-flex my-1">
						...
						<div class="pl-3 pr-2 flex-grow-1 text-break-all text-dark">
							다운로드시 <b><?php echo $board['bo_download_point'] * (-1) ?></b> 포인트 차감
							<?php if($down_term) { ?>
								<?php if($is_down_term) { ?>
									(<?php echo $is_down_term ?> 까지 무료)
								<?php } else { ?>
									(<?php echo $down_term ?>일 동안 다운로드 이력 유지)
								<?php } ?>
							<?php } else { ?>
								(최초 1회 차감 / 재다운로드시 무료)
							<?php } ?>
...
$(function() {
	...
	$("a.view_file_download").click(function() {
		...
		<?php } else { ?>
			<?php if($down_term) { ?>
				var msg = "파일을 다운로드 하시면 포인트가 차감(<?php echo number_format($board['bo_download_point']) ?>점)됩니다.\n\n포인트는 게시물당 <?php echo $down_term ?>일 동안 다시 다운로드 하셔도 중복하여 차감하지 않습니다.\n\n그래도 다운로드 하시겠습니까?";	
			<?php } else { ?>
